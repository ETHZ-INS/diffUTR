...
## UTR- and CDS-specific analyses

By default, the above workflow detects differences in bin usage, whether these
are from coding sequences (CDS) or untranslated regions (UTR). To looks 
specifically for one or the other, it is not necessary to re-run the wrappers,
but simply to perform the gene-level aggregation using different filters. This 
can be done using the `geneLevelStats()` function, which accepts the arguments 
`excludeTypes` and `includeTypes` to decide which bin types to include in the 
gene-level estimates. For example:

```{r}
# only look for changes in CDS bins:
cds <- geneLevelStats(rse, includeTypes="CDS", returnSE=FALSE)
head(cds)
# only look for changes in UTR bins:
utr <- geneLevelStats(rse, includeTypes="UTR", returnSE=FALSE)
# only look for changes in bins that _could be_ UTRs:
utr <- geneLevelStats(rse, excludeTypes=c("CDS","non-coding"), returnSE=FALSE)
```
...
Differential analysis also works in the same way. By default, this will look 
for changes in _any_ bin type; if you want to restrict the gene-level 
aggregation to UTR bins, you can use:
...
By default, the gene-level statistics computed use all bin types. However, for 
the purpose of differential 3' UTR analysis, it is useful to restrict exclude 
coding sequence (CDS) bins (keeping bins that could be both) or limit 
ourselves to 3' UTR bins. This can be done by recomputing the gene-level 
...
We can then plot again the top genes, indicating that we are in differential 
UTR mode, which will use signed and width-weighted aggregation:
...
usage of bins which are generally not included in most transcripts of that gene
 (low count density), and are therefore less likely to be relevant. On the other
 hand, some genes show massive expression of a single bin which might (or might
 not) represent a different, unannotated transcript.
