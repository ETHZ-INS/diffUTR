...
* An improved version of `r BiocStyle::Biocpkg("limma")`'s `diffSplice` method (see `?diffSpliceWrapper`)
* `r BiocStyle::Biocpkg("edgeR")`'s `diffSpliceDGE` method (see `?diffSpliceDGEWrapper`)
...
We load diffUTR as well as the `r BiocStyle::Biocpkg("SummarizedExperiment")` package on which it depends:
...
Unfortunately, the [polyASite](https://polyasite.unibas.ch/atlas) database does not contain APA sites for the rat. A somewhat older similar database, [PolyA_DB](https://exon.apps.wistar.org/PolyA_DB/v3/), does include the rat, however with an obsolete annotation; for convenience we lifted it over to Rno6, and made it available in the package data:
...
```{r}
rse <- diffSpliceWrapper(rse, design = ~condition, excludeTypes = c("CDS","non-coding"))
```
